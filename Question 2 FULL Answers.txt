...
2.4 - Slack Style Update :
	...
	👋 **Pipeline Update – Company Activity Dashboard**

		• **What's changing:** We're adding incremental date filtering to the daily job so it only processes today's data instead of re-scanning the full history — this should bring the runtime down from 3+ hours to under 30 mins.

		• **Dedup fix incoming:** We've identified that API retries were causing duplicate event counts for some company/date combinations. A deduplication layer is being added — expect the `events` numbers for a handful of companies to slightly decrease and become more accurate.

		• **Temporary caveat:** During today's deployment window (~30 mins), the dashboard may show stale data from yesterday's run. No data will be lost.

		• **Watch out for:** If you spot a company where today's metrics look lower than yesterday's, that's likely the dedup fix correcting inflated numbers — not a pipeline failure. Flag anything that looks off and we'll investigate same-day. 🙏
